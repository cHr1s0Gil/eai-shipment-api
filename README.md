...
ERP에서 WMS로 출고 지시가 전달되는 상황을 가정한 Spring Boot 기반 EAI 백엔드 포트폴리오 프로젝트입니다.

이 프로젝트는 ERP로부터 출고 지시 데이터를 수신하고, DB에 저장한 뒤 조회, 상태 관리, 실패 처리, 재처리, 스케줄 기반 전송, Kafka 기반 비동기 처리를 구현하는 것을 목표로 합니다.

현재 MVP에서는 실제 ERP와 WMS를 별도 시스템으로 구현하지 않습니다. 대신 하나의 Spring Boot 애플리케이션 안에서 EAI API 서버, Kafka Producer, Kafka Consumer를 함께 구현하여 전체 출고 지시 처리 흐름을 시뮬레이션합니다.
...
실무 EAI 운영 업무에서 자주 다루는 다음 흐름을 Spring Boot 프로젝트로 재구성했습니다.
...
현재는 WMS 서버가 없으므로 `ShipmentDispatchConsumer`가 같은 Spring Boot 애플리케이션 안에서 WMS 처리 결과를 시뮬레이션합니다. 실제 운영 구조에서는 EAI 서버가 Producer, WMS 또는 WMS 연계 서버가 Consumer가 되는 형태로 분리할 수 있습니다.
...
- 처리 성공/실패 상태 반영
- 오래된 PROCESSING 상태 timeout 처리
- 공통 API 응답 포맷
- 전역 예외 처리
- Swagger API 문서
- H2 Console
- Kafka UI
- 파일 기반 애플리케이션 로그 / SQL 로그 분리
- 서비스 책임 분리 리팩토링
- 서비스/컨트롤러 테스트
...
DB는 MVP 기준으로 `shipment_request` 단일 테이블을 유지합니다. Java 코드에서는 의미 단위별 값 객체를 `@Embeddable` / `@Embedded`로 분리했습니다.
...
`FAILED` 상태가 되면 실패 사유는 `message`에 저장하고, 필요 시 실패 당시 payload는 `errorPayload`에 저장합니다. `FAILED`가 아닌 상태로 변경되면 실패 메시지와 payload는 정리됩니다.

## 서비스 책임 분리

초기에는 `ShipmentRequestService` 하나에 등록, 조회, dispatch, Consumer 결과 처리, timeout 처리 로직이 함께 있었습니다.

Kafka와 Scheduler 기능이 추가되면서 서비스 책임을 다음과 같이 분리했습니다.
...
Producer와 Consumer는 Kafka 입출력에 집중하고, 실제 업무 판단과 DB 상태 변경은 Service 계층에서 처리하도록 분리했습니다.
...
즉, 서버 시작 시간 기준 `fixedRate`가 아니라 서버 시간 기준 매 5분 정각에 실행됩니다.

스케줄러는 두 가지 작업을 수행합니다.
...
다건 처리 시 일부 건에서 예외가 발생해도 전체 배치를 중단하지 않고 다음 건을 계속 처리합니다.

### PROCESSING timeout

`PROCESSING` 상태가 오래 유지되는 건은 자동 재전송하지 않습니다.

이유는 EAI 서버가 WMS의 실제 수신 여부를 확정할 수 없기 때문입니다. 단순 서버 순단인지, WMS 처리 지연인지, Kafka 처리 문제인지 알 수 없는 상태에서 자동 재전송하면 중복 출고 위험이 있습니다.
...
향후 운영 수준에서는 DB 또는 설정 저장소 기반 제어로 확장할 수 있습니다.
...
주요 로그에는 `dispatchBatchId`를 남겨 스케줄러 실행 단위와 Kafka 메시지 흐름을 추적할 수 있도록 했습니다.
...
- 엔티티를 API 응답에 직접 노출하지 않고 DTO로 변환합니다.
- 상태 변경은 Transaction 내부 JPA dirty checking을 활용합니다.
- Kafka 발행은 비동기이므로 dispatch API 응답은 최종 성공이 아니라 `PROCESSING` 상태를 반환합니다.
- Kafka 최종 처리 결과는 Consumer가 별도로 반영합니다.
- 장기 PROCESSING 건은 자동 재전송하지 않고 FAILED로 전환하여 중복 출고 위험을 줄입니다.
- 스케줄러는 cron 기반으로 매 5분 정각에 실행되도록 설정했습니다.
- 운영성 API는 향후 Spring Security로 인가 처리를 적용할 예정입니다.
...
- Kafka consumer offset / consumer group 운영 관점 문서화
